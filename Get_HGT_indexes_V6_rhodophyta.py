...
# Writen in 20 of April 2020 (Finished on 25 of April) by L. Felipe Benites during the plague - mod last version for Rhodophyte genome (Gracilaria dominguensis) in September 2020.
...
# This is what it has in the file "Query	qlen	sseqid	slen	qstart	qend	sstart	send	Evalue	Bitscore	score	length	qcovhsp	Identity	staxids	stitle	salltitles	Superkingdom	Scientific_name"
...
# Usage: python Get_HGT_indexes_V6.py taxon_blast_file.csv (or change tsv to csv and delimiter from '\t' to ',')
# Real usage: python Get_HGT_indexes_V6_rhodophyta.py gd_proteins_diamond_lineages_renamed.tsv
...
data.loc[data.Superkingdom.str.contains("Viruses;|;Bacteria;|;Archaea;",na=False,case=True), 'category'] = 'nontarget'

data.loc[data.Superkingdom.str.contains(";Eukaryota;",na=False,case=True), 'category'] = 'target'

# remove taxa or group which is same from genome of interest
data = data[data.Superkingdom.str.contains(";Gracilariaceae;") == False]
...
# get best Evalue for indexed sequences (Query=query name)
Best_Evalues3= pd.pivot_table(data, index= 'Query', values='Superkingdom', aggfunc='first')
...
cols_final = pd.concat( [drop_met_nonmet, Best_Evalues1,Best_sp_name2,Best_Evalues3], axis=1)
...
# reorder cols
cols_final = cols_final[['Alien index (AI)','AI > 30','HGT index (hU)','hU >= 30','Evalue','Top species Evalue','Superkingdom']]
